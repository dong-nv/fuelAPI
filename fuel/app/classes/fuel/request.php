...
class Request extends \Fuel\Core\Request
{
	...
	public function __construct($uri, $route = true, $method = null)
	{
		...
		logger(\Fuel::L_DEBUG, 'Creating a new '.(static::$main==null ? 'main' : 'HMVC').' Request with URI = "'.$this->uri->get().'"', __METHOD__);
	...
	public function execute($method_params = null)
	{
		...
		logger(\Fuel::L_DEBUG, 'Called', __METHOD__);
		...
		if ( ! static::$main)
		{
			logger(\Fuel::L_DEBUG, 'Setting main Request', __METHOD__);
			static::$main = $this;
		}
